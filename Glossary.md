...
N: 全圆彩虹 /待定 /未完成
D: 天空中突然出现七色同心圆带的现象。这种彩虹在消失时呈现出带状塌陷的几何特征。 可以用数学公式解释如下。 XXXXXXXXXXXXXXXXXXXXXXXX 假设最外层圆的半径为 r=R，则在 r=R/2 处的环被表示为（*），其中 n=1。随着 r 变小，即 R/2，n 的值无限趋近于 ∞，随着 r 的增大，n 的值趋近于 0，并最终崩溃消失。彩虹是一种神秘的自然现象，很少被观察到。
...
N: 简谐运动 /未完成
D: 简谐振动是提取圆运动的一维分量的运动。例如，假设圆的半径为A，角速度为ω，时刻为t，则简谐振动物体的位置x可由以下公式表示：x=Asin(ωt)。此外，对其进行两次微分可得到ma=-Kx的运动方程式（其中m、a是运动物体的质量和加速度，K是常数）。简谐振动物体的特点是受力取决于位置。附在弹簧上的物体的运动方程式为ma=-kx，因此弹簧的运动也是简谐振动。
